...
export async function scrapeUrl(url: string): Promise<FirecrawlScrapedData> {
  try {
...
export async function extractPriceFromUrl(url: string): Promise<{
  price: number | null;
  currency: string;
  rawText: string;
}> {
...
export async function checkStockFromUrl(url: string): Promise<{
  inStock: boolean;
  indicators: string[];
}> {
  const scraped = await scrapeUrl(url);
